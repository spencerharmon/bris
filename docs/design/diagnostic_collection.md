...
Every `media[]` entry may carry `checksum_sha256` — a
lowercase-hex SHA-256 digest of the raw file — in addition to
`size_bytes`. It is optional (additive to schema v1, so older
clients keep working with size-only validation); when present,
the collector recomputes it from the bytes actually received on
ingest and rejects a mismatch with 400. This is what lets a
stored submission be *proven*, not merely assumed, byte-identical
to what the device captured.
  ...
  2026/05/13/
    ...
    01HXYZ...02/
      ...
      calibration/
        ...
        ...
      media/
  .staging/                   # transient; atomic-finalize working area
index.sqlite                # list/filter index, rebuildable
collector.log               # operator log; no PII
```

Placement is role-driven: a `media[]` item with `role:
"pbris_log"` lands at the submission root as `pbris.log`; an
item with `role: "calibration_frame"` or `"intrinsics_toml"` (or
any item in a `submission_kind: "calibration"` submission) lands
under `calibration/`; everything else lands under `media/`.
...
`bris_collector index-rebuild` to rebuild it from scratch by
walking `submissions/**/manifest.json` (and any `deleted_at.txt`
...
+ media written, sizes and any declared SHA-256 checksums
verified) in a private staging directory under
`<data-root>/submissions/.staging/<id>/` and only becomes visible
at its final `submissions/<yyyy>/<mm>/<dd>/<id>/` path via a
single atomic `rename(2)`. A crash before the rename leaves
nothing at the final path — only an orphaned entry under
`.staging/` (not yet reaped by an automated sweep; a stale
`.staging/<id>/` is safe to remove by hand, it was never
indexed). The SQLite insert happens only after the rename
succeeds; a crash between rename and insert is repaired with
`bris_collector index-rebuild`.

Soft-delete: `bris_collector soft-delete <id>` writes a sidecar
`deleted_at.txt` (an RFC3339 UTC timestamp) next to the
submission's `manifest.json` — the manifest itself is never
rewritten — and flips `soft_deleted_at` in the mirror row, hiding
the submission from the default review UI's listing. Files
remain on disk for the retention window (default 30 days,
configurable). Hard-delete after the window is a separate,
explicit operator action: `bris_collector retention-sweep
[--retention-days N] [--dry-run]`. Neither soft-delete nor
retention-sweep ever runs implicitly — not at startup, not as a
deploy side effect, not from the HTTP surface — precisely because
submitted captures are precious primary data.
